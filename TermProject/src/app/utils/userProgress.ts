...
export const calculateLevel = (xp: number): { level: number; title: string; xpToNext: number; progress: number } => {
...
export const calculateXPGain = (score: number, totalQuestions: number): number => {
  const baseXP = 10;
...
// TODO (Person 1/2 — Backend): Replace these stubs with real API calls
export const saveUserData = async (_userData: UserData): Promise<void> => {
  // TODO: POST /api/users/:id or PATCH /api/users/:id/progress
};

export const loadUserData = async (): Promise<UserData | null> => {
  // TODO: GET /api/users/me (using JWT from auth)
  return null;
};

export const clearUserData = (): void => {
  // TODO: handled by auth logout route (Person 1)
};
